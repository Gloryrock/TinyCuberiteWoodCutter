function OnPlayerBreakingBlock(a_Player, a_BlockX, a_BlockY, a_BlockZ, a_BlockFace, aBlockType, aBlockMeta)
	-- Check if the user should only be allowed to use survival with this plugin
	if (gConfig.SurvivalOnly and not a_Player:IsGameModeSurvival()) then
		return false
	end
	...
	if (gConfig.AxeRequired and not ItemCategory.IsAxe(EquippedItem.m_ItemType)) then
		return false
	end
	...
	local Collection = gCollectionClass(a_Player, BlockType, BlockMeta)
	...
	if (gConfig.ReplantSapling) then
		if (math.random() < gConfig.ReplantSaplingRate) then
